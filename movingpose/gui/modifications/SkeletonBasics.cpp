...
// store skeleton data for text file
...
{    
    ...
    // stores value from text file used to comunicate between C++ and Python
    string tp = "NA";

    // load text file content to tp
    fstream newfile;
    newfile.open("record_frame_count.txt", ios::in);
    ...
    Vector4 skeletonPos[NUI_SKELETON_POSITION_COUNT];
    ...
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i)
    {
        ...
        skeletonPos[i] = skel.SkeletonPositions[i];
        if (skel.eSkeletonPositionTrackingState[i] == NUI_SKELETON_POSITION_NOT_TRACKED) {
            skeletonPos[i].w = 0;
        }

        // if desired, save skeleton data to vector
        if (limit != -1) {
            if (total_frames <= limit) {
                string jf = to_string(total_frames);
                string jw = to_string(i + 1); //to_string(skeletonPos[i].w);
                string jx = to_string(skeletonPos[i].x);
                string jy = to_string(skeletonPos[i].y);
                string jz = to_string(skeletonPos[i].z);
                ...
                SetStatusMessage(L"Collecting Data");
            }
            else {
                SetStatusMessage(L"Done Collecting :)");
            }
        }
        else {
            SetStatusMessage(L"No Limit Set; Not Collecting!");
        }
    }
    ...
    if(total_frames == limit) {
        ofstream myfile;
        myfile.open("record_frame_count.txt");
        myfile << "DONE-RECORDING";
        myfile.close();
        total_frames = 1;
        limit = -1;
